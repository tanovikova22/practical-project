  ...
  <v-container>
    ...
    <template v-if="users !==null">
      <div class="text-center">
        <v-pagination v-model="pageNumber" :length="numberOfPages" :total-visible="7"></v-pagination>
      </div>
      <v-row align="center">
        Show:
      ...
      <v-simple-table fixed-header>
        <template v-slot:default>
          ...
          <tbody>
            <tr v-for="(user, idx) in showenItems" :key="user.id">
              <td>{{filterItems.indexOf(user)+1}}</td>
              <template v-if="editUserIndex === idx">
                <td>
                  <v-text-field required label="Name" v-model="user.name"></v-text-field>
                </td>
                <td>
                ...
                <td>
                  <v-btn depressed color="light green" @click="editUserIndex = null">
                    Done
                  ...
                  <div class="text-center">
                    <v-menu offset-x>
                      <template v-slot:activator="{ on, attrs }">
                        <v-btn class="primary" depressed v-bind="attrs" v-on="on">Actions</v-btn>
                      </template>
                      <v-list>
                        <v-list-item>
                          <v-btn
...
export default {
  data: () => ({
    search: "",
    pageNumber: 1,
    emailVariety: ["gmail.com", "mail.ru", "mail.ua"],
    emailChoice: "",
    editUserIndex: null,
    usersToShow: 3
  }),
  ...
  computed: {
    ...
    filterItems() {
      ...
      if (this.emailChoice) {
        filteredUsers = filteredUsers.filter(element => {
          let splitedEmails = element.email.split("@");
          return splitedEmails[1] === this.emailChoice;
        });
      }
